...
export default function FinanceDetailSidebar() {
	...
	const { donationCache, detailLoading, fetchDonation, updateDonation, deleteDonation } = useFinanceStore();
				...
				<div className="h-full flex flex-col overflow-hidden">
					...
					<div className="flex items-center justify-between px-6 py-4 border-b border-gray-100 shrink-0">
						<div className="min-w-0">
							{donation ? (
								<>
									<h2 className="text-lg font-bold text-gray-900 truncate">Donation #{donation.id}</h2>
							...
							<div>
								...
								{editing ? (
									...
									<div className="p-6 space-y-6">
										{/* Donation Info */}
										<div>
											<h3 className="text-xs font-semibold text-gray-400 uppercase tracking-wider mb-3">Donation Info</h3>
											<div className="space-y-3">
												<DetailRow label="Amount" value={`₹${donation.amount.toLocaleString()}`} highlight />
												<DetailRow
													label="Date"
												...
												/>
												<DetailRow label="Payment Method" value={PAYMENT_LABELS[donation.payment_method] || donation.payment_method} />
												<DetailRow label="Transaction Ref" value={donation.transaction_reference || ' - '} />
										...
										{/* Volunteer Info */}
										<div>
											<h3 className="text-xs font-semibold text-gray-400 uppercase tracking-wider mb-3">Volunteer Info</h3>
											<div className="space-y-3">
										...
										<div>
											...
											<div className="space-y-3">
												...
												<DetailRow label="Created" value={new Date(donation.created_at).toLocaleString('en-IN')} />
												<DetailRow label="Last Updated" value={new Date(donation.updated_at).toLocaleString('en-IN')} />
											</div>
										</div>
									</div>
								)}
					...
					<div className="absolute inset-0 z-10 flex items-center justify-center bg-black/40">
						<div className="bg-white rounded-xl shadow-xl max-w-sm w-full mx-6 p-6">
							...
							<p className="text-sm text-gray-600 mt-2">
								Are you sure you want to delete the ₹{donation?.amount.toLocaleString()} donation from{' '}
								<strong>{donation?.volunteer_name}</strong>? This cannot be undone.
							</p>
...
function DetailRow({ label, value, highlight = false }: { label: string; value: React.ReactNode; highlight?: boolean }) {
	return (
		<div className="flex justify-between items-start">
			...
			<span className={`text-sm text-right ml-4 ${highlight ? 'text-lg font-bold text-gray-900' : 'font-medium text-gray-900'}`}>
				{value}
			</span>
		</div>
	);
